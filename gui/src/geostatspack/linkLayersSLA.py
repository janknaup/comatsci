...
class layerLinkSLA(layerLinkBase):
	...
	def linkAction(self):
		...
		progressWindow.setLabelText("generating simple link atoms")
		progressWindow.show()
		(self.embeddedGeometry,results)=self.geo.layersubgeometry(self.QMZComboBox.currentItem()).SLALinkedGeometry(self.geo.layersubgeometry(self.PCHRComboBox.currentItem()),progressWindow.setTotalSteps,progressWindow.setProgress,distscale=ladf)
		progressWindow.hide()
		return results
