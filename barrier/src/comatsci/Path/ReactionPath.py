...
##	def readfmgpath(self,name,checkCompat=True,GeoConstructor=Geometry.Geometry):
##		"""read the path from a single, multi-geometry .fmg file
...
##			print 'Parse xml file "%s" and build DOM... ' % (name),
##			sys.stdout.flush()
##		dom = xml.dom.minidom.parse(name)
##		if self.verbosity >= constants.VBL_DEBUG2:
##			print 'done.'
##		fmg=dom.getElementsByTagName("fmg")
##		if self.verbosity >= constants.VBL_DEBUG2:
##			print 'extract geometry, trjsteps and trjinfo elements'
##		geometries=fmg[0].getElementsByTagName("geometry")
##		trjsteps=fmg[0].getElementsByTagName("trjstep")
##		trjinfo=fmg[0].getElementsByTagName("trjinfo")
##		if self.verbosity >= constants.VBL_DEBUG2:
##			print 'iterate through geometries ',
##		for i in geometries:
##			self.geos.append(GeoConstructor())
##			self.geos[-1].handlegeometry_dom(i)
##			if self.verbosity >= constants.VBL_DEBUG2:
##				print '.',
##				sys.stdout.flush()
...
##					else:
##						raise
##			# fix for memory leakage: cleanup unused doms, do not rely on garbage collection
##			i.unlink()
##		# fix for memory leakage: cleanup unused doms, do not rely on garbage collection
##		if self.verbosity >= constants.VBL_DEBUG2:
##			print 'done.'
...
##			self.handletrjinfo_dom(trjinfo[0])
##		# kill possibly stored spline representation
##		self.splineRep=None
##		if self.verbosity >= constants.VBL_DEBUG2:
